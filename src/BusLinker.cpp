...
BusLinker::BusLinker(SoftwareSerial *bus, long baudRate) {
  port = bus;
  baud = baudRate;
}

uint8_t BusLinker::checkSum(uint8_t bufferLength, uint8_t *buffer) {
...
int BusLinker::time(uint32_t len) {
  return ((len * 10000) / baud)+ 10;  // time for transmission
}

int BusLinker::sendCommand(CommandType command, uint8_t id, uint8_t numOfParams, uint8_t *params) {
...
int BusLinker::read(CommandType command, uint8_t *params, int numOfParams, uint8_t id) {
